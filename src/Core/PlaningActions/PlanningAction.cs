...
namespace Core.PlaningActions
{
    public class PlanningAction<T> : ICloneable
    {
        ...
        private readonly List<Func<IState<T>, bool>> prejudicates = new List<Func<IState<T>, bool>>();
        private readonly List<Action<IState<T>>> postActions = new List<Action<IState<T>>>();
        ...
        public PlanningAction<T> AddPrejudicate(Func<IState<T>, bool> prejudicate)
        {
            prejudicates.Add(prejudicate);
            return this;
        }
        public PlanningAction<T> AddPostAction(Action<IState<T>> postAction)
        {
            postActions.Add(postAction);
            return this;
        }

        public bool CanExecute(IState<T> state)
        {
        ...
        public IState<T> Migrate(IState<T> state)
        {
            var newState = state.Clone() as IState<T>;
            Execute(newState);
            return newState;
        }
